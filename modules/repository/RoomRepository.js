...
module.exports = {
    getRoomsByUserId: function (userId, callback) {
        databaseConfig.getSession().query('SELECT id,user_id,name,max_guests,room_code,room_name,enabled FROM room r WHERE r.user_id = ? ORDER BY created_date desc', userId, (err, result) => {
            if (err) {
                console.log(err);
